...
func TestAccGroupEntitlement_Create(t *testing.T) {
	...
	principalName := os.Getenv("AZDO_TEST_AAD_GROUP_PRINCIPAL_NAME")
	resource.ParallelTest(t, resource.TestCase{
		PreCheck:     func() { testutils.PreCheck(t, &[]string{"AZDO_TEST_AAD_GROUP_PRINCIPAL_NAME"}) },
		Providers:    testutils.GetProviders(),
		CheckDestroy: checkGroupEntitlementDestroyed,
		Steps: []resource.TestStep{
			{
				Config: testutils.HclGroupEntitlementResource(principalName),
				Check: resource.ComposeTestCheckFunc(
					checkGroupEntitlementExists(strings.ReplaceAll(principalName, "\\\\", "\\")),
...
func checkGroupEntitlementExists(expectedPrincipalName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if !strings.EqualFold(strings.ToLower(*groupEntitlement.Group.PrincipalName), strings.ToLower(expectedPrincipalName)) {
			return fmt.Errorf("GroupEntitlement with ID=%s has PrincipalName=%s, but expected Name=%s", resource.Primary.ID, *groupEntitlement.Group.PrincipalName, expectedPrincipalName)
		}
