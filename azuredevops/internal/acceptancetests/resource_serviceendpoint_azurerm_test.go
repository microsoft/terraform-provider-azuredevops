...
func TestAccAzureRMServiceEndpointSharedProjectIDs(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: hclAzureRMServiceEndpointWithOneSharedProject(projectName, serviceEndpointName),
				Check: resource.ComposeTestCheckFunc(
					testutils.CheckServiceEndpointExistsWithName(tfSvcEpNode, serviceEndpointName),
					resource.TestCheckResourceAttrSet(tfSvcEpNode, "project_id"),
					resource.TestCheckResourceAttr(tfSvcEpNode, "shared_project_ids.#", "1"),
					resource.TestCheckResourceAttrPair(tfSvcEpNode, "shared_project_ids.0", "azuredevops_project.shared1", "id"),
				),
			},
			{
				Config: hclAzureRMServiceEndpointWithTwoSharedProjects(projectName, serviceEndpointName),
				Check: resource.ComposeTestCheckFunc(
					testutils.CheckServiceEndpointExistsWithName(tfSvcEpNode, serviceEndpointName),
					resource.TestCheckResourceAttrSet(tfSvcEpNode, "project_id"),
					resource.TestCheckResourceAttr(tfSvcEpNode, "shared_project_ids.#", "2"),
					resource.TestCheckResourceAttrPair(tfSvcEpNode, "shared_project_ids.0", "azuredevops_project.shared1", "id"),
					resource.TestCheckResourceAttrPair(tfSvcEpNode, "shared_project_ids.1", "azuredevops_project.shared2", "id"),
				),
			},
			{
				Config: hclAzureRMServiceEndpointWithNoSharedProjects(projectName, serviceEndpointName),
				Check: resource.ComposeTestCheckFunc(
					testutils.CheckServiceEndpointExistsWithName(tfSvcEpNode, serviceEndpointName),
					resource.TestCheckResourceAttrSet(tfSvcEpNode, "project_id"),
...
func hclAzureRMServiceEndpointWithOneSharedProject(projectName, serviceEndpointName string) string {
	return fmt.Sprintf(`
resource "azuredevops_project" "test" {
  name = "%[1]s"
}

resource "azuredevops_project" "shared1" {
  name = "%[1]s-shared1"
}
...
resource "azuredevops_serviceendpoint_azurerm" "test" {
  ...
  shared_project_ids = [azuredevops_project.shared1.id]
}
`, projectName, serviceEndpointName)
}

func hclAzureRMServiceEndpointWithTwoSharedProjects(projectName, serviceEndpointName string) string {
	return fmt.Sprintf(`
resource "azuredevops_project" "test" {
  name = "%[1]s"
}
...
resource "azuredevops_project" "shared2" {
  name = "%[1]s-shared2"
}
...
resource "azuredevops_serviceendpoint_azurerm" "test" {
  ...
  shared_project_ids = [
    azuredevops_project.shared1.id,
    azuredevops_project.shared2.id
  ]
}
`, projectName, serviceEndpointName)
}

func hclAzureRMServiceEndpointWithNoSharedProjects(projectName, serviceEndpointName string) string {
	return fmt.Sprintf(`
resource "azuredevops_project" "test" {
  name = "%[1]s"
}
