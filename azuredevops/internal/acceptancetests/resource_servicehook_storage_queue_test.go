...
func TestAccServicehookStorageQueue_basic(t *testing.T) {
	...
	resourceType := "azuredevops_servicehook_storage_queue"
	tfCheckNode := resourceType + ".test"
	resource.ParallelTest(t, resource.TestCase{
		PreCheck:     func() { testutils.PreCheck(t, nil) },
		Providers:    testutils.GetProviders(),
		CheckDestroy: CheckServicehookDestroyed,
		Steps: []resource.TestStep{
			{
				Config: testutils.HclServicehookStorageQeueueResourceWithPipelinesPublisher(projectName, accountKey, queueName, resultFilter),
...
func TestAccServicehookStorageQueue_accountKeyError(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:      testutils.HclServicehookStorageQeueueResourceWithPipelinesPublisher(projectName, "accountkey", "testqueue", "Canceled"),
...
func CheckServicehookDestroyed(s *terraform.State) error {
	for _, resource := range s.RootModule().Resources {
		if resource.Type != "azuredevops_servicehook_storage_queue" {
			continue
		}
