...
/*
func TestSecurityNamespace_GetIndentitiesFromACL_HandleError(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	// ReadIdentities
	var errMsg = "@@ReadIdentities@@failed"
	identityClient.
		EXPECT().
		ReadIdentities(clients.Ctx, gomock.Any()).
		Return(nil, fmt.Errorf(errMsg)).
		Times(1)
	idList, err := sn.getIndentitiesFromACL(&projectAccessControlList[0])
	assert.NotNil(t, err)
	assert.EqualError(t, err, errMsg)
	assert.Nil(t, idList)
}

func TestSecurityNamespace_GetIndentitiesFromACL_VerifyReturn(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	var descriptors string
	var descriptorList []string
	for _, ace := range *projectAccessControlList[0].AcesDictionary {
		descriptorList = append(descriptorList, *ace.Descriptor)
	}
	descriptors = strings.Join(descriptorList, ",")
	identityClient.
		EXPECT().
		ReadIdentities(clients.Ctx, IsReadIdentitiesArgs(identity.ReadIdentitiesArgs{
			Descriptors: &descriptors,
		})).
		Return(&projectIdentityList, nil).
		Times(1)
